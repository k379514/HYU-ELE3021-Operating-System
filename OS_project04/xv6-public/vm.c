...
int CoW_copyuvm(pde_t *pgdir, pde_t *newpgdir, uint sz){
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if (!(*pte & PTE_P))
      return -1;
    pa = PTE_ADDR(*pte);
    *pte &= ~PTE_W; // change parent's physical pages with read only
    flags = PTE_FLAGS(*pte);

    if (mappages(newpgdir, (void*)i, PGSIZE, pa, flags) < 0) // mapping to parent's physical pages
      goto bad;
...
void CoW_handler(void){
  ...
  if (pte && !(*pte & PTE_W)) {
    ...
    if(get_refc(pa) == 1){ // if refc is 1, just modify permission
      *pte |= PTE_W;
    } else {
      char *mem = kalloc();
      ...
      *pte = V2P(mem) | PTE_P | PTE_W | PTE_U; // point to new page
